import pandas as pd
import numpy as np
data = pd.read_csv('../dataset/ISCX_Botnet-Testing.pcap_Flow.csv', low_memory=False,encoding="gbk")
# data = data[0:50000]
print(data.shape)
...
#Type=HTTP-2
data.loc[indx_Sogou, 'Label'] = 2
data.loc[indx_Virut, 'Label'] = 2

#Type=P2P-3
data.loc[indx_Zeus_1, 'Label'] = 3
data.loc[indx_Zeus_2, 'Label'] = 3
data.loc[indx_Zeus_3, 'Label'] = 3
data.loc[indx_bin_Zeus, 'Label'] = 3

data.loc[indx_Weasel_master, 'Label'] = 3
data.loc[indx_Weasel_bot, 'Label'] = 3

data.loc[indx_zero_access_1, 'Label'] = 3
data.loc[indx_zero_access_2, 'Label'] = 3

data.loc[indx_Smoke_bot, 'Label'] = 3

data.to_csv('../dataset/testing_label_protocoltype.csv',encoding='utf-8')